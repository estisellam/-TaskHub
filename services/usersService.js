...
async function updateUserBlockStatus(id, isBlocked) {
    return await funcUsersRepository.updateUserBlockStatus(id, isBlocked);
}

async function createUser(data) {
    if (!data.user_name || !data.email || !data.first_name || !data.last_name || !data.password) {
    ...
    if (exUserName) throw new Error("Username already exists");
...
async function updateUser(id, user) {
    if (!user.user_name || !user.email || !user.first_name || !user.last_name) {
        throw new Error("All fields are required");
    }
...
async function login(data) {
    const user = await funcUsersRepository.getUserByUsername(data.user_name);
    if (!user) {
        throw new Error("Invalid username or password");
    }

    const passwordData = await funcPasswordsRepository.getPasswordHash(user.id);
    const isMatch = await bcrypt.compare(data.password, passwordData.hash_password);
    if (!isMatch) {
        throw new Error("Invalid username or password");
    }

    return user; // מחזיר את המשתמש כולל שדה ה-is_blocked שלו!
}
